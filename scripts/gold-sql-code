...
Script Purpose：
    ...
    The Gold layer represents the final dimension and fact sales (Star Schema).

	Each view performs transformations and combines data from the Silver layer to
    provide a clean, enriched, and business-ready dataset.
...
SELECT  
		...
		CASE WHEN cst_gndr != 'n/a' THEN cst_gndr -- CRM is the master for gender in datasource.
